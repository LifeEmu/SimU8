...
static uint8_t codeSegHandler(uint32_t address, uint8_t data, bool isWrite) {
	...
	return *((uint8_t *)CodeMemory + (address & ( (CODE_MIRROW_MASK << 16) | 0xffff)));
}
...
static uint8_t Seg5Handler(uint32_t address, uint8_t data, bool isWrite) {
	// dseg5 on CWI is mapped to cseg0
	return codeSegHandler(address & 0xffff, data, isWrite);
}
